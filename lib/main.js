...
async function inPrimaryWindow(callback) {
  await lumine.workspace.focusPrimaryWindow();
  return callback();
}

function getPanes() {
  const allPanes = lumine.workspace.getCenter().getTiledPanes(),
...
function getActiveTiledPane() {
  const center = lumine.workspace.getCenter();
  const panes = center.getTiledPanes();
  const activePane = center.getActivePane();
  return panes.includes(activePane) ? activePane : (panes[0] ?? null);
}
...
async function addUntilNPanes(n, horizontal) {
  ...
  if (paneCount === 0) {
    const activePane = getActiveTiledPane();
    if (activePane) {
...
function redistributeItems() {
  ...
  const activeItem = getActiveTiledPane()?.getActiveItem();
  const allItems = [];
...
async function ensurePanes(n, horizontal, detail) {
  ...
  const allCenterPanes = lumine.workspace.getCenter().getTiledPanes();
  if (allCenterPanes.some((p) => p.parent?.parent?.constructor?.name === "PaneAxis")) {
    ...
    while (panes.length > 1) {
      ...
      panes = lumine.workspace.getCenter().getTiledPanes();
    }
  }
...
async function ensureGrid(n, detail) {
  await ensurePanes(n, false);
  const panes = getPanes();
